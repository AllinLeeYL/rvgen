...
pub struct Cli {
    ...
    pub seed: Option<i64>,
    /// Allow privileged generation (including the opt-in CSR instruction class).
...
/// Only 1, true, and yes (case insensitive) are true.
...
impl Cli {
    ...
    pub fn with_config(self, config: Config) -> Result<Options> {
        ...
        params.validate()?;
        let num_elfs = self.num_elfs.or(config.num_elfs).unwrap_or(1);
        if num_elfs == 0 {
            return Err("num_elfs must be greater than zero".into());
        }
        params
            .seed
            .checked_add(i64::try_from(num_elfs - 1)?)
