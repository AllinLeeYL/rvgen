...
# FIXME you should tolerate it
__NO_INTERACTION_MINSTRET = True
...
__TOLERATE_NAXRISCV_CLINT = False

def is_tolerate_naxriscv_clint():
    return __TOLERATE_NAXRISCV_CLINT
...
def tolerate_bug_for_eval_reduction(design_name: str, is_activate: bool = True):
    ...
    if design_name == "boom":
        ...
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_BOOM_MINSTRET"
        )
    elif design_name == "rocket":
        __TOLERATE_ROCKET_MINSTRET = is_activate
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_ROCKET_MINSTRET"
        )
    elif design_name == "cva6":
        __TOLERATE_CVA6_MHPMCOUNTER = is_activate
        # __TOLERATE_CVA6_FDIVS_FLAGS = is_activate
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_CVA6_MHPMCOUNTER"
        )
    elif design_name == "kronos":
        __TOLERATE_KRONOS_MINSTRET = is_activate
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_KRONOS_MINSTRET"
        )
    elif design_name == "picorv32":
        __TOLERATE_PICORV32_FENCE = is_activate
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_PICORV32_FENCE"
        )
    elif design_name == "vexriscv":
        __TOLERATE_VEXRISCV_MINSTRET = is_activate
        print(
            f"WARNING: Tolerating one bug for evaluating reduction (activate: {is_activate}): __TOLERATE_VEXRISCV_MINSTRET"
        )
    else:
        raise Exception(
            "Unknown design name in __tolerate_bug_for_eval_reduction: " + design_name
        )
...
def tolerate_bug_for_bug_timing(design_name: str, bug_name: str, is_activate: bool):
    ...
    global __TOLERATE_NAXRISCV_CLINT
    ...
    if bug_name == "p1":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_PICORV32_READNONIMPLCSR"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_PICORV32_READNONIMPLCSR"
            )
    elif bug_name == "p2":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_PICORV32_MISSINGMANDATORYCSRS"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_PICORV32_MISSINGMANDATORYCSRS"
            )
    elif bug_name == "p3":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_PICORV32_WRITEHPM"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_PICORV32_WRITEHPM"
            )
    elif bug_name == "p4":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_PICORV32_READHPM_NOCSRRS"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_PICORV32_READHPM_NOCSRRS"
            )
    elif bug_name == "p5":
    ...
    # Vexriscv
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_IMPRECISE_FCVT"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_IMPRECISE_FCVT"
            )
    elif bug_name == "v2":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_DOUBLE_TO_FLOAT"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_DOUBLE_TO_FLOAT"
            )
    elif bug_name == "v4":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_SINGLE_PRECISION"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_SINGLE_PRECISION"
            )
    elif bug_name == "v5":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_FLE_FEQ_RET1"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_FLE_FEQ_RET1"
            )
    elif bug_name == "v6":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_FLT_RET0"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_DEPENDENT_FLT_RET0"
            )
    elif bug_name == "v7":
        ...
        assert design_name == "vexriscv-v8-9-v15", (
            "Bug v8 is only for vexriscv-v8-9-v15"
        )
        __TOLERATE_VEXRISCV_IMPRECISE_FCVT = is_activate
        __TOLERATE_VEXRISCV_FMIN = is_activate
        __TOLERATE_VEXRISCV_DOUBLE_TO_FLOAT = is_activate
        ...
        assert design_name == "vexriscv-v8-9-v15", (
            "Bug v9 is only for vexriscv-v8-9-v15"
        )
        __TOLERATE_VEXRISCV_DEPENDENT_SINGLE_PRECISION = is_activate
        __TOLERATE_VEXRISCV_IMPRECISE_FCVT = is_activate
        __TOLERATE_VEXRISCV_FMIN = is_activate
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_FPU_DISABLED"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_FPU_DISABLED"
            )
    elif bug_name == "v11":
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_FPU_LEAK"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_FPU_LEAK"
            )
    # elif bug_name == 'v12':
    ...
    #         print('INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_MHPMCOUNTERN')
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_VEXRISCV_MINSTRET"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_VEXRISCV_MINSTRET"
            )
    elif bug_name == "v14":
        assert design_name == "vexriscv-v8-9-v15", (
            "Bug v14 is only for vexriscv-v8-9-v15"
        )
        __TOLERATE_VEXRISCV_DEPENDENT_SINGLE_PRECISION = is_activate
        if is_activate:
            print(
    ...
    # Kronos
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_KRONOS_READBADCSR"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_KRONOS_READBADCSR"
            )
    elif bug_name == "k4":
    ...
    # CVA6
        ...
        if is_activate:
            print(
                "WARNING: Tolerating one bug for timing: __TOLERATE_CVA6_SINGLE_PRECISION"
            )
        else:
            print(
                "INFO: De-tolerating one bug for timing: __TOLERATE_CVA6_SINGLE_PRECISION"
            )
    elif bug_name == "c3":
        assert design_name == "cva6", f"Bug {bug_name} is only for cva6"
    ...
    # NAXRISCV
        ...
        assert design_name == "naxriscv", "Bug n1 is only for naxriscv"
        __TOLERATE_NAXRISCV_CLINT = is_activate
        if is_activate:
            print("WARNING: Tolerating one bug for timing: __TOLERATE_NAXRISCV_CLINT")
        else:
            print("INFO: De-tolerating one bug for timing: __TOLERATE_NAXRISCV_CLINT")
