...
def instruc_rtype(opcode: int, rd: int, funct3: int, rs1: int, rs2: int, funct7: int) -> int:
    if __debug__:
        assert opcode >= 0
        assert rd >= 0
        assert funct3 >= 0
...
# @param funct3: uint8_t
# @param rs1: uint8_t
def instruc_itype(opcode: int, rd: int, funct3: int, rs1: int, imm: int) -> int:
    if __debug__:
        assert opcode < (1 << 7)
        assert rd < 32
        assert funct3 < 8
...
def instruc_utype(opcode: int, rd: int, imm: int):
    ...
    return opcode | rd << rd_offset | imm31_12 << imm31_12_offset
...
def instruc_jtype(opcode: int, rd: int, imm: int):
    ...
    return opcode | rd << rd_offset | immparts << immparts_offset
...
def instruc_r4type(
    opcode: int, rd: int, funct3: int, rs1: int, rs2: int, rs3: int, funct2: int
):
    if __debug__:
        assert opcode < (1 << 7)
        assert rd < 32
...
def instruc_crtype(opcode: int, rs2: int, rds1: int, funct4: int):
    ...
    return (
        opcode | (rs2 << rs2_offset) | (rds1 << rds1_offset) | (funct4 << funct4_offset)
    )
...
def instruc_csstype(opcode: int, rs2: int, funct3: int, imm: int):
    ...
    return (
        opcode | (rs2 << rs2_offset) | (imm << imm_offset) | (funct3 << funct3_offset)
    )
...
def instruc_catype(
    opcode: int, rs2prime: int, funct2: int, rds1prime: int, funct6: int
):
    if __debug__:
        assert opcode < 4
        assert rds1prime < 8
...
def instruc_cjtype(opcode: int, funct3: int, imm: int):
    ...
    return opcode | (imm << imm_offset) | (funct3 << funct3_offset)
