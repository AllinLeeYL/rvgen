...
use rvgen::utils::elfbuilder::{ElfBuilder, ElfSection, SHF_ALLOC, SHF_WRITE};
...
#[test]
fn matches_python_elf_bytes() {
    for (is_64bit, empty, populated) in [
        (
            false,
            include_bytes!("fixtures/empty32.bin").as_slice(),
            include_bytes!("fixtures/sections32.bin").as_slice(),
        ),
        (
            true,
            include_bytes!("fixtures/empty64.bin").as_slice(),
            include_bytes!("fixtures/sections64.bin").as_slice(),
        ),
    ] {
        assert_eq!(
            ElfBuilder
                .build(&[ElfSection::new(".text", vec![])], is_64bit, 0x8000_0000)
                .unwrap(),
            empty
        );
        assert_eq!(
            ElfBuilder
                .build(&sections(), is_64bit, 0x8000_0000)
                .unwrap(),
            populated
        );
    }
