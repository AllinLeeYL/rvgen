...
fn binary_reads_config_and_runs_outside_repository() {
    ...
    assert_executable(&std::fs::read(dir.0.join("from config.elf")).unwrap());
...
fn batch_output_names_config_overrides_and_seed_range() {
    ...
    for args in [
        vec!["--num-elfs", "0"],
        vec!["--num-elfs", "2", "--seed", "9223372036854775807"],
    ] {
...
#[test]
fn cli_size_counts_workload_instructions_and_seed_changes_bytes() {
    let dir = TempDir::new();
    let mut workloads = Vec::new();
    for (size, blocks, seed) in [
        (0, 7, 42),
        (17, 7, 42),
        (17, 7, 43),
        (17, 7, 42),
        ...
        let bytes = std::fs::read(dir.0.join("output.elf")).unwrap();
        ...
        assert_eq!(
            workloads.last().unwrap().len(),
            workloads[0].len() + size * 4
        );
    }
    assert_ne!(workloads[1], workloads[2]);
    assert_eq!(workloads[1], workloads[3]);
}
